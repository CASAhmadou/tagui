...
chrome_step('Page.navigate',{url: 'https://github.com/kelaberetiv/TagUI'}); sleep(1000);
techo('https://github.com/kelaberetiv/TagUI' + ' - ' + chrome.getTitle() + '\n');});
...
casper.then(function() {{techo('read (//*[@class="Link--muted"])[3] to license_type');
casper.waitFor(function check() {return check_tx('(//*[@class="Link--muted"])[3]');},
function then() {license_type = chrome.fetchText(tx('(//*[@class="Link--muted"])[3]')).trim();},
function timeout() {chrome.capture('/home/cas/TagUI_Linux/tagui/flows/samples/2_github_error.png');
this.echo('ERROR - cannot find (//*[@class="Link--muted"])[3]').exit(); this.wait(0);});}});

casper.then(function() {this.echo(''+license_type+'');});
...
casper.then(function() {{techo('click //get-repo');
casper.waitFor(function check() {return check_tx('//get-repo');},
function then() {chrome.click(tx('//get-repo'));},
function timeout() {chrome.capture('/home/cas/TagUI_Linux/tagui/flows/samples/2_github_error.png');
this.echo('ERROR - cannot find //get-repo').exit(); this.wait(0);});}});

casper.then(function() {{techo('click //*[contains(@href, "master.zip")]');
casper.waitFor(function check() {return check_tx('//*[contains(@href, "master.zip")]');},
function then() {chrome.click(tx('//*[contains(@href, "master.zip")]'));},
function timeout() {chrome.capture('/home/cas/TagUI_Linux/tagui/flows/samples/2_github_error.png');
this.echo('ERROR - cannot find //*[contains(@href, "master.zip")]').exit(); this.wait(0);});}});

// Wait 15 seconds to give the download time to complete on slow networks
casper.then(function() {techo('wait 15');});
casper.then(function() {casper.wait(15000, function() {});});
